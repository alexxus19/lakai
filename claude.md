...
- **Multi-select drag stale-capture fix**: `ReorderDropDelegate` changed `selectedIDs: Set<UUID>` (value copy) to `selectedIDsProvider: () -> Set<UUID>` (closure) so `dropEntered` always reads the live selection instead of a snapshot captured at view-body evaluation time, which could be stale if SwiftUI did not re-register the drop delegate during an active drag session
